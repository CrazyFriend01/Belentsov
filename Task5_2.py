...
class InputConnect:
    ...
    def filter_by_params(params, vacancies_objects):
        ...
        if params[0] == "Оклад":
            filtered_vacancies = list(
                filter(lambda x: float(x.salary.salary_from) <= float(params[1]) <= float(x.salary.salary_to),
                       vacancies_objects))
            ...
            filtered_vacancies = list(
                filter(lambda x: currencies[x.salary.salary_currency] == (params[1]), vacancies_objects))
    ...
    def formatter(vacancies_objects):
        ...
        def get_experience(row):
            return work_experience[row.experience_id]

        def get_date(row):
            return datetime.strptime(row.published_at, "%Y-%m-%dT%H:%M:%S%z").strftime("%d.%m.%Y")
